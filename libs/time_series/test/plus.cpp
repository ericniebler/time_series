...
void test_series_plus()
{
    ...
    ss2 += ss1;
    BOOST_CHECK_EQUAL(3, std::distance(seq::begin(ss2), seq::end(ss2)));
    BOOST_CHECK_EQUAL(3, seq::elements(ss2)(*seq::begin(ss2)));
    BOOST_CHECK_EQUAL(1, rrs::offset(rrs::runs(ss2)(*seq::begin(ss2))));
    BOOST_CHECK_EQUAL(8, seq::elements(ss2)(*++seq::begin(ss2)));
...
// test_series_plus_2
//
void test_series_plus_2()
{
    // check addition of two characteristic series yields
    ...
    BOOST_CHECK_EQUAL(2, rrs::get_at(ss3, 4));
    BOOST_CHECK_EQUAL(2, rrs::get_at(ss3, 5));
...
///////////////////////////////////////////////////////////////////////////////
// test_series_plus_floating_point
//
void test_series_plus_floating_point()
{
    // check addition of two characteristic series yields
    // a piecewise_constant time series
    characteristic_unit_series<int,int,double> ss1(-3.33, 3.33);
    characteristic_unit_series<int,int,double> ss2(0.0, 6.66);

    piecewise_constant_series<int,int,double> expected;
    make_ordered_inserter(expected)
        (1, -3.33, 0.0)(2, 0.0, 3.33)(1, 3.33, 6.66)
    .commit();

    piecewise_constant_series<int,int,double> actual = ss1 + ss2;
...
{
    test_suite *test = BOOST_TEST_SUITE("series plus test");

    test->add(BOOST_TEST_CASE(&test_series_plus));
    test->add(BOOST_TEST_CASE(&test_series_plus_2));
    test->add(BOOST_TEST_CASE(&test_series_plus_floating_point));
